...
- Wildcard CORS is restricted to loopback development. Non-loopback listeners require explicit trusted origins.
- `/docs`, `/redoc`, and `/openapi.json` support local onboarding. Production Compose and Helm profiles set `AGENT_BOM_DISABLE_DOCS=1` so those routes return 404.
