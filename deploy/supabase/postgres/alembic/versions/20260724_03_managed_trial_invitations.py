...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_managed_trial_tenants_due "
        "ON managed_trial_tenants(state, trial_ends_at, cleanup_after)"
    )
    ...
    op.execute("ALTER TABLE managed_trial_tenants ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE managed_trial_tenants FORCE ROW LEVEL SECURITY")
    op.execute(
        """
        DO $$
        BEGIN
    ...
    op.execute(
        ...
        BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM pg_policies
            WHERE schemaname = current_schema()
              AND tablename = 'managed_trial_tenants'
              AND policyname = 'managed_trial_tenants_tenant_isolation'
          ) THEN
            CREATE POLICY managed_trial_tenants_tenant_isolation ON managed_trial_tenants
              USING (public.abom_rls_bypass() OR tenant_id = public.abom_current_tenant())
              WITH CHECK (public.abom_rls_bypass() OR tenant_id = public.abom_current_tenant());
          END IF;
        END $$
    ...
    op.execute(
        ...
        BEGIN
          IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'agent_bom_app') THEN
            ...
            GRANT SELECT, INSERT, UPDATE, DELETE ON managed_trial_tenants TO agent_bom_app;
          END IF;
        END $$
        """
    )
    op.execute(
        ...
        VALUES ('managed_trial_tenants', 1, now())
