...
| `correlation-graph-live.png` | `/security-graph?lens=attack-path&scan=reference-evidence-correlation-v1&cve=CVE-2023-4863&capture=1`, then Graph | Interactive reference-lab graph with the real advisory, modeled service and data asset, and remediation context | README graph proof; captured from the application, not an illustrated diagram |
...
| `correlation-receipts-mobile-live.png` | `/security-graph?lens=attack-path&scan=reference-evidence-correlation-v1&correlation=1&capture=1` | Reference-lab outcome and actions at 390 by 844 without horizontal overflow | Mobile readability contract; not a separate evidence run |
