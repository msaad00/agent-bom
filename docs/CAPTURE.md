...
| `mesh-path-live.png` | `/mesh?capture=1` | Capture the bounded agent mesh path view showing selected agent -> MCP servers -> reachable packages/tools -> prioritized findings | Keeps first-pass triage focused before operators drill into the full graph canvas |
