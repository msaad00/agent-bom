...
| `cloud` | Cloud posture + estate inventory for AWS/Azure/GCP, CIS benchmarks, and cloud registry sweeps. Group: `scan` (all configured clouds), `aws`/`azure`/`gcp` aliases, `inventory`, `registry-scan` (ECR/ACR/GAR sweep), `resilience`, `side-scan` (AWS EBS + Azure Managed Disk + GCP Persistent Disk CLI executors), `side-scan-capabilities` (honest per-provider surface; no live credentialed smoke claimed), and CWPP `runtime-evidence-ingest`. |
...
| `attest` | Sign/verify tenant-bound SBOM DSSE PAE, per-instance MCP scan attestations (`attest mcp sign|verify`), and compliance evidence bundles (`attest compliance-verify`) with explicit Ed25519 trust policies; legacy embedded-key envelopes remain untrusted. |
...
|---|---|---|
| AWS / Azure / GCP | `agent-bom cloud ...` | provider APIs expose estate inventory, IAM, network posture, CIS checks, and registry sweeps; the `side-scan` CLI executor ships for AWS EBS, Azure Managed Disk, and GCP Persistent Disk (opt-in via `AGENT_BOM_SIDESCAN`), while `side-scan-capabilities` reports the honest per-provider surface — no live credentialed smoke is claimed yet |
...
Both paths normalize into the same `Finding` and `UnifiedGraph` model; the
