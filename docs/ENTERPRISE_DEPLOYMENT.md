...
**Tracing:** every API response includes `X-Request-ID`, `X-Trace-ID`, `X-Span-ID`, and W3C `traceparent`. If your ingress or collector already sends `traceparent` or `tracestate`, `agent-bom` preserves the upstream trace context and continues the chain. Set `AGENT_BOM_OTEL_TRACES_ENDPOINT` to export API request spans over OTLP/HTTP, and use `AGENT_BOM_OTEL_TRACES_HEADERS` for collector auth headers when needed.
...
docker run --rm \
  ...
  -v $(pwd):/workspace:ro \
  agentbom/agent-bom:0.75.15 agents --format json
```
