...
- **Cloud provider API resilience:** `agent-bom cloud resilience --format json` reports the declared provider pagination, retry/backoff, partial-failure, and 10k-resource validation target for AWS, Azure, GCP, Snowflake, Databricks, Hugging Face, OpenAI, W&B, MLflow, Nebius, CoreWeave, and Ollama. The command is credential-free posture metadata, not proof that the target was achieved; provider entries marked `partial` mean executable provider-scale evidence is still outstanding, not that inventory is silently trusted.
...
- **Proxy-to-control-plane mTLS posture** — delegated mTLS through ingress, Envoy, or the service mesh is the recommended production default. Set `AGENT_BOM_PROXY_CONTROL_PLANE_MTLS_MODE=delegated` plus client-CA and evidence refs, then verify `proxy_control_plane_mtls` in `/v1/auth/policy`. Non-mesh installs can use app-native uvicorn mTLS with `AGENT_BOM_TLS_CERT_FILE`, `AGENT_BOM_TLS_KEY_FILE`, `AGENT_BOM_TLS_CLIENT_CA_FILE`, and `AGENT_BOM_TLS_REQUIRE_CLIENT_CERT=1`. The app still requires trusted-proxy shared-secret attestation and issuer pinning for header-derived identity so certificate-verified proxies cannot be bypassed by raw `X-Agent-Bom-*` headers.
