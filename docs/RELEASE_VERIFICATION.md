...
```bash
cosign verify-blob agent_bom-0.77.0-py3-none-any.whl \
  --bundle agent_bom-0.77.0-py3-none-any.whl.sigstore.json \
...
```bash
cosign verify-blob agent_bom-0.77.0.tar.gz \
  --bundle agent_bom-0.77.0.tar.gz.sigstore.json \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  --certificate-identity-regexp 'https://github.com/msaad00/agent-bom/.github/workflows/release.yml@.*'
```
...
```bash
head -n 5 agent_bom-0.77.0-py3-none-any.whl.intoto.jsonl
```
...
```bash
gh attestation verify agent_bom-0.77.0-py3-none-any.whl \
