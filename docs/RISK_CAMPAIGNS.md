...
When remediation removes every current member, the campaign retires from the active list. `GET /v1/campaigns/verification-queue?limit=100` preserves the tenant-scoped re-verification handoff across reloads and restarts with its durable title, original member count, workflow status, and current version. Follow the opaque `next_cursor` while `has_more` is true; pages use stable campaign-ID keyset ordering and never load the full queue.
