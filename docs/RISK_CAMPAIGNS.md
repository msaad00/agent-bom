...
Assign an owner and SLA with `PATCH /v1/campaigns/{campaign_id}`, create tickets through a stored ticketing connection, and then call `POST /v1/campaigns/{campaign_id}/verify` with the current workflow `version`. Verification is server-owned: it compares the durably stored original member IDs with a complete current canonical-findings snapshot, returning the remaining IDs or marking the campaign verified and done.
