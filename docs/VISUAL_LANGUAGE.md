...
| `persona-value-{light,dark}.svg` | Four buyer personas in a readable 2x2 map — AI engineer, Security engineer, GRC / audit, Leadership / CISO (security engineering ≠ GRC / audit), the same four names and order as the README "Who it is for" table; each card includes capabilities, a literal first command, an outcome, and a proof artifact/state | README "Who it is for" |
