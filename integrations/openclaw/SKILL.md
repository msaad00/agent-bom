...
metadata:
  ...
  install:
    pipx: agent-bom
    pip: agent-bom
    docker: ghcr.io/msaad00/agent-bom:0.54.0
  openclaw:
    requires:
      bins: []
      env: []
...
```bash
docker run --rm ghcr.io/msaad00/agent-bom:0.54.0 scan
```
...
- **Sigstore signed**: `agent-bom verify agent-bom@0.54.0`
- **3,027+ tests** with automated security scanning (CodeQL + OpenSSF Scorecard)
