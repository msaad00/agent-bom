...
compatibility: >-
  ...
  Registry data (1013 MCP servers) is bundled in-package with zero network calls.
metadata:
  author: msaad00
  ...
  install:
    pipx: agent-bom
    pip: agent-bom
    docker: ghcr.io/msaad00/agent-bom:0.97.0
  openclaw:
    requires:
      bins: []
      env: []
...
bundled 1013-entry MCP server registry.
