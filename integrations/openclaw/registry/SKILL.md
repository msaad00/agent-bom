...
description: >-
  MCP server security registry and trust assessment — look up servers in the 1112-entry
...
metadata:
  ...
  openclaw:
    ...
    os:
      ...
      - linux
      - windows
    data_flow: "Purely local. Registry data (1112 MCP server metadata records) is bundled in the package. Lookups are in-memory string matches. Skill trust analysis parses user-provided SKILL.md content passed as a string argument."
    file_reads:
...
Look up MCP servers in the 1112-entry server security metadata registry, assess skill
...
| Tool | Description |
|------|-------------|
| `registry_lookup` | Look up MCP server in the 1112-entry security metadata registry |
...
|----------|-------------|
| `registry://servers` | Browse the 1112-entry MCP server security metadata registry |
