...
# The manifest carries a human-readable "Generated … on YYYY-MM-DD" stamp. That
# date must never make ``--check`` report the file stale on a later day when the
# content is otherwise byte-identical, so the stamp date is normalised out before
# the staleness comparison (the written file still records the real date).
_STAMP_RE = re.compile(r"(<!-- Generated by [^\n]*? on )\d{4}-\d{2}-\d{2}( -->)")


def _normalize_stamp(text: str) -> str:
    return _STAMP_RE.sub(r"\g<1><date>\g<2>", text)
...
def render(snapshot: dict) -> str:
    ...
    mcp_default_tools = _metric(metrics, "MCP default tools")
    ...
    rest_cell = (
        f"{rest_operations} operations across {rest_route_modules} route modules "
        f"(+ {rest_ws_routes} WebSocket routes) — see `docs/openapi/v1.json`"
    )
    regen_line = (
        f"`{version}` — regenerate metrics with `python scripts/product_metrics_snapshot.py --write` "
        f"and this file with `python scripts/generate_agent_capability_manifest.py --write`."
    )

    mcp_surface = (
        f"{mcp_default_tools} default scan tools; explicit full catalog: "
        f"{mcp_tools} tools, {mcp_resources} resources, {mcp_prompts} workflow prompts"
    )
...
| Area | Shipped today |
|---|---|
| MCP security tools | {mcp_surface} |
| REST API | {rest_cell} |
...
def main() -> None:
    ...
    if args.check:
        ...
        if _normalize_stamp(OUT.read_text(encoding="utf-8")) != _normalize_stamp(content):
