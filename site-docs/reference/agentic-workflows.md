...
| GitHub Actions | `uses: msaad00/agent-bom@v0.98.2` with SARIF upload enabled | Runs in CI with repository-scoped token permissions. Fork PR behavior depends on GitHub security policy. | `agent-bom-results.sarif`, pull-request summary, code-scanning alert category. | Branch protection, required code scanning, artifact retention. |
...
agent-bom scan . -f html -o agent-bom-report.html
agent-bom scan . -f sarif -o agent-bom-results.sarif
```
