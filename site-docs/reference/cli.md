...
- `secrets` is local by default. For source you are authorized to check against providers, run `agent-bom secrets . --validate-credentials --format json -o secrets.json`. This explicitly sends found GitHub user tokens (`ghp_`, `gho_`, `ghu_`) to `GET https://api.github.com/user` and Stripe keys to `GET https://api.stripe.com/v1/balance`. Provider response bodies are not read or saved. Inspect each finding's `validation_status`: `valid` means authentication succeeded at check time; `invalid` means the provider returned 401; `unknown` covers unsupported credentials, permissions, rate limits, transport errors, and exhausted check budgets. Default scans omit this field. `--offline` rejects `--validate-credentials` before scanning.
- Validation uses at most 20 distinct credential checks per scan, a two-second timeout per network operation, no retries or redirects, and no ambient proxy configuration. Duplicate supported credentials share a digest-only cache. AWS access keys remain `unknown` because a key ID alone cannot authenticate; key-pair association is not supported. PII is never validated. Invalid or unknown credentials remain findings with their original severity and exit behavior: remove the exposed secret and review its history. This flag is available only on the dedicated `secrets` CLI; API, scheduled, MCP, and general `scan` paths do not enable live validation.
- `secrets` reports directory-policy and linked-worktree exclusions as `pruned_directories`, separately from repository `ignored_paths`. Exclusions, unreadable files, and exhausted traversal budgets set `complete: false`. The console displays coverage warnings; both JSON and console exit `2` for incomplete coverage when no finding already requires exit `1`. To inspect an excluded `env` or `build` subtree, name it explicitly: `agent-bom secrets ./env --format json`. Counts describe excluded ancestors, not unenumerated files inside them.
- Shared filesystem discovery also records exclusions and traversal limits in `coverage_warnings`, including scanners that return only discovered paths. `excluded_count` counts skipped ancestors or limit events, not files inside an unvisited tree. A stderr warning identifies the coverage gap without exposing local paths. For a full project report, run `agent-bom scan --project . --format json -o report.json`, inspect `coverage_warnings`, then scan an excluded subtree explicitly if it is in scope.
- Repository scans submitted through the API retain the same coverage evidence in `ai_inventory.secrets`, including when there are zero findings. Secret-scan coverage warnings also appear as `scanner_coverage_gap` issues from `secret-scan` in `scan_run`, making its outcome `partial`. This applies to reports with dependency inventory and reports containing only static findings; a detected secret alone does not make execution incomplete.
