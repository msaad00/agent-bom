"""Scan pipeline orchestration — extracted from server.py for modularity.

Contains the ``ScanPipeline`` tracker, the blocking ``run_scan_sync()``
function that drives discovery→extraction→scanning→enrichment→analysis→output,
the fleet-sync helper, and the background cleanup loop.
"""
...
def _sync_scan_agents_to_fleet(agents: list) -> None:
    ...
    always populated after every scan.
    """
...
# ─── Main scan function (runs in thread pool) ───────────────────────────────
...
_STUCK_JOB_TIMEOUT = 1800  # 30 minutes
