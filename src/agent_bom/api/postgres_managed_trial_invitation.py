...
from agent_bom.api.tenant_lifecycle import MANAGED_TRIAL_CLEANUP_GRACE, MANAGED_TRIAL_DURATION
...
class PostgresManagedTrialInvitationStore:
    ...
    def _from_row(row: tuple[Any, ...]) -> ManagedTrialInvitation:
        return ManagedTrialInvitation(
            ...
            expires_at=row[6],
            accepted_at=row[7],
            verified_subject=str(row[8]) if len(row) > 8 and row[8] is not None else None,
        )
    ...
    def issue(self, invitation: ManagedTrialInvitation, *, team_name: str) -> None:
        ...
        try:
            with _tenant_connection(self._pool) as conn:
                ...
                trial_ends_at = invitation.created_at + MANAGED_TRIAL_DURATION
                conn.execute(
                    ...
                    (
                        ...
                        trial_ends_at,
                        trial_ends_at + MANAGED_TRIAL_CLEANUP_GRACE,
                        invitation.created_at,
                    ),
                )
                conn.execute(
