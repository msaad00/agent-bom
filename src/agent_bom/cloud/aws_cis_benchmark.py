...
def _check_1_11(iam_client: Any) -> CISCheckResult:
    ...
    if users_with_initial_keys:
        result.status = CheckStatus.FAIL
        result.evidence = (
            f"{len(users_with_initial_keys)} user(s) with access keys created at setup: "
            f"{', '.join(users_with_initial_keys[:5])}"
        )
...
def _check_1_13(iam_client: Any) -> CISCheckResult:
    ...
    if multi_key_users:
        result.status = CheckStatus.FAIL
        result.evidence = (
            f"{len(multi_key_users)} user(s) with multiple active access keys: "
            f"{', '.join(multi_key_users[:5])}"
        )
...
def _check_1_19(ec2_client: Any) -> CISCheckResult:
    ...
    try:
        ...
        if no_role:
            result.status = CheckStatus.FAIL
            result.evidence = (
                f"{len(no_role)} running instance(s) without IAM instance profile: "
                f"{', '.join(no_role[:5])}"
            )
            if len(no_role) > 5:
...
def _check_2_1_3(s3_client: Any) -> CISCheckResult:
    ...
    if no_mfa_delete:
        result.status = CheckStatus.FAIL
        result.evidence = (
            f"{len(no_mfa_delete)} bucket(s) without MFA Delete: "
            f"{', '.join(no_mfa_delete[:5])}"
        )
...
def _check_5_5(ec2_client: Any) -> CISCheckResult:
    ...
    if open_sgs:
        result.status = CheckStatus.FAIL
        result.evidence = (
            f"{len(open_sgs)} security group(s) allow unrestricted ingress to all ports: "
            f"{', '.join(open_sgs[:5])}"
        )
        if len(open_sgs) > 5:
            result.evidence += f" (+{len(open_sgs) - 5} more)"
