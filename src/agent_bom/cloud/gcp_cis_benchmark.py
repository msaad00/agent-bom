    ...
    try:
        ...
        if contact_list:
            ...
            if missing:
                ...
                result.status = CheckStatus.PASS
                result.evidence = f"Essential contacts configured with {len(contact_list)} contact(s) covering SECURITY, TECHNICAL, and BILLING."
        else:
            result.status = CheckStatus.FAIL
...
def _check_2_3(project_id: str) -> CISCheckResult:
    ...
    try:
        ...
        found = any(
            any(kw in (m.filter or "").lower() for kw in filter_keywords)
            for m in metrics
        )
...
def _check_2_4(project_id: str) -> CISCheckResult:
    ...
    try:
        ...
        found = any(
            all(kw in (m.filter or "").lower() for kw in filter_keywords)
            for m in metrics
        )
        ...
        for _zone, response in agg:
            for instance in response.instances or []:
                ...
                if sas:
                    ...
                    if email.endswith("-compute@developer.gserviceaccount.com") and "https://www.googleapis.com/auth/cloud-platform" in scopes:
                        failing.append(getattr(instance, "name", "unknown"))
...
def _check_7_2(project_id: str) -> CISCheckResult:
    ...
    try:
        ...
        if failing:
            result.status = CheckStatus.FAIL
            result.evidence = f"BigQuery datasets without default table expiration ({len(failing)}/{len(datasets)}): {', '.join(failing[:10])}"
            result.resource_ids = failing
        else:
            result.status = CheckStatus.PASS
