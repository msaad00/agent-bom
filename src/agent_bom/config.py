...
# A separate bounded pool polls durable report claims on each API replica.
API_REPORT_WORKERS = _int("AGENT_BOM_API_REPORT_WORKERS", 2)  # Zero creates an API-only report queue producer.
API_REPORT_LEASE_SECONDS = _int("AGENT_BOM_API_REPORT_LEASE_SECONDS", 60)
API_REPORT_MAX_ATTEMPTS = _int("AGENT_BOM_API_REPORT_MAX_ATTEMPTS", 3)
# Explicit acknowledgement of a shared mounted artifact directory.
REPORT_ARTIFACT_SHARED = _bool("AGENT_BOM_REPORT_ARTIFACT_SHARED", False)
