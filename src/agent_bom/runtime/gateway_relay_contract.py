"""Gateway pure-relay contract.

Isolates the **pure HTTP JSON-RPC forward** surface from policy / detector /
audit orchestration, so the relay loop is replaceable without forking product
semantics. ADR-009 evaluated a Go sidecar against this contract and retired it;
the boundary stays because it is what made that removal a one-line change.
...
def build_gateway_relay_transport(client: Any, *, max_bytes: int = MAX_GATEWAY_RELAY_MESSAGE_BYTES) -> GatewayRelayTransport:
    """Build the relay transport used to forward a JSON-RPC message upstream."""
