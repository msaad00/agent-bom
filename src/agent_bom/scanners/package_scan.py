...
def _scanners_patchable(name: str) -> Any:
    """Resolve a test-patchable symbol from ``agent_bom.scanners``."""
    import agent_bom.scanners as scanners_mod

    return getattr(scanners_mod, name)


def _emit_scan_warning(message: str) -> None:
    _scanners_patchable("record_scan_warning")(message)
        ...
        with _scan_cache_init_lock:
            if _scan_cache_instance is None:
                try:
                    ...
                    _emit_scan_warning("scan cache unavailable")
...
def _scan_packages_local_db(packages: list[Package]) -> tuple[int, set[str]]:
    ...
    try:
        ...
        _emit_scan_warning("local vulnerability DB freshness check failed")
        return 0, set()
    ...
    try:
        ...
        _emit_scan_warning("local vulnerability DB unavailable")
        return 0, set()
