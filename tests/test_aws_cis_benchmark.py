...
class TestCheck11:
    def test_pass_contact_details_present(self):
        client = MagicMock()
        client.get_contact_information.return_value = {
            "ContactInformation": {"FullName": "Alice Admin", "PhoneNumber": "+15551234567"}
        }
    ...
    def test_fail_incomplete_contact(self):
        client = MagicMock()
        client.get_contact_information.return_value = {
            "ContactInformation": {"FullName": "", "PhoneNumber": "+15551234567"}
        }
        result = _check_1_1(client)
        assert result.status == CheckStatus.FAIL
...
class TestCheck12:
    def test_pass_security_contact_registered(self):
        client = MagicMock()
        client.get_alternate_contact.return_value = {
...
class TestCheck17:
    ...
    def test_fail_root_recently_used(self):
        ...
        client.get_credential_report.return_value = {
            "Content": (
                "user,password_last_used\n"
                f"<root_account>,{recent.isoformat()}\n"
            ).encode()
        }
        result = _check_1_7(client)
        assert result.status == CheckStatus.FAIL
...
class TestCheck39:
    def test_pass_all_vpcs_covered(self):
        ...
        client.describe_flow_logs.return_value = {
            "FlowLogs": [{"ResourceId": "vpc-1", "FlowLogStatus": "ACTIVE"}]
        }
...
class TestCheck310:
    def test_pass_write_event_logging_enabled(self):
        ...
        client.describe_trails.return_value = {
            "trailList": [{"TrailARN": "arn:trail", "Name": "main"}]
        }
        client.get_event_selectors.return_value = {
            "EventSelectors": [
                {
...
class TestCheck41:
    def test_pass_filter_exists(self):
        ...
        paginator = MagicMock()
        paginator.paginate.return_value = [
            {"metricFilters": [{"filterPattern": "{ $.errorCode = AccessDenied }"}]}
        ]
        client.get_paginator.return_value = paginator
...
class TestCheck42:
    ...
    def test_fail_partial_match(self):
        ...
        paginator = MagicMock()
        paginator.paginate.return_value = [
            {"metricFilters": [{"filterPattern": "{ $.eventName = ConsoleLogin }"}]}
        ]
        client.get_paginator.return_value = paginator
        result = _check_4_2(client)
        assert result.status == CheckStatus.FAIL
