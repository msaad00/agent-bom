...
def test_overview_excludes_skipped_pushed_findings_from_current_evidence() -> None:
    from agent_bom.api.routes.overview import _current_scan_severity, _estate_rollup
    from agent_bom.api.server import ScanJob, ScanRequest
    ...
    skipped = ScanJob(
        ...
        result={
            "scan_skipped": True,
            "dry_run": True,
            "findings": [{"id": "fake", "severity": "critical", "soc2_tags": ["CC6.1"]}],
        },
    )

    estate = _estate_rollup([skipped])

    assert estate["unique_findings"] == 0
    assert estate["severity"]["critical"] == 0
    assert estate["latest_scan_at"] is None
    assert _current_scan_severity([skipped])["critical"] == 0
