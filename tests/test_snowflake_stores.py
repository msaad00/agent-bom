...
class TestSnowflakeJobStore:
    ...
    def test_get_found(self, mock_connect):
        from agent_bom.api.server import JobStatus, ScanJob
        job = self._make_job()
...
class TestSnowflakePolicyStore:
    ...
    def test_get_policies_for_agent_filters(self, mock_connect):
        from agent_bom.api.policy_store import GatewayPolicy
        p1 = GatewayPolicy(
            policy_id="p1", name="agent-specific",
            bound_agents=["agent-a"], enabled=True,
            updated_at=datetime.now(timezone.utc).isoformat(),
        )
        p2 = GatewayPolicy(
            policy_id="p2", name="global",
            enabled=True,
            updated_at=datetime.now(timezone.utc).isoformat(),
        )
        cur = _mock_cursor(fetchall_val=[
            (p1.model_dump_json(),), (p2.model_dump_json(),),
        ])
        conn = _mock_connection(cursor=cur)
        mock_connect.return_value = conn
        store = self._make_store()
