...
export function GraphEvaluationSummary({ evaluation }: GraphEvaluationSummaryProps) {
  ...
  return (
    <section
      ...
      className="mt-3 rounded-xl border border-outline bg-background/70 p-3"
    ...
    >
      <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex shrink-0 items-baseline gap-2 rounded-lg border border-outline bg-surface/70 px-3 py-2">
          <span className="text-2xl font-semibold tabular-nums text-foreground">{Math.round(evaluation.score)}</span>
          <span className="text-[10px] uppercase tracking-[0.16em] text-ink-tertiary">score</span>
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
        {evaluation.dimensions.map((dimension) => {
          ...
          return (
            <div key={dimension.id} className="rounded-lg border border-outline bg-surface/60 p-2.5">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-xs font-semibold tabular-nums text-foreground">{Math.round(dimension.score)}</span>
              </div>
              ...
              <p className="mt-2 break-words text-[11px] leading-4 text-ink-tertiary">{dimension.detail}</p>
            </div>
          );
        })}
      </div>
      ...
      {evaluation.warnings.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2">
          {evaluation.warnings.slice(0, 3).map((warning) => (
            <span
              key={warning}
              className="inline-flex max-w-full items-start gap-1.5 rounded-lg border border-amber-500/25 bg-amber-500/10 px-2.5 py-1 text-[11px] leading-4 text-amber-800 dark:text-amber-100"
            >
