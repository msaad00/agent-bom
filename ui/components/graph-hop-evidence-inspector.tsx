...
export function GraphHopEvidenceInspector({
  ...
  return <section aria-label="Hop evidence inspector" className="min-w-0 rounded-xl border border-outline bg-surface">
    <div className="flex flex-wrap items-center justify-between gap-3 p-3">
      <div className="min-w-0">
        <h3 className="text-sm font-semibold">Evidence at each hop</h3>
        <p className="text-xs text-ink-secondary">{filtered.length} of {rows.length} relationships · inspect one hop at a time</p>
      </div>
      <label className="flex min-w-0 items-center gap-2 rounded-lg border border-outline px-2 py-1.5">
    ...
    <p className="px-3 pb-3 text-xs text-ink-secondary">Relationships describe the recorded path. An observed call does not prove permission, exploitation, or a successful downstream action.</p>
    <ol className="divide-y divide-outline border-t border-outline">
      {visible.map(({ source, target, index, receipt, status }) => {
        ...
        return <li key={`${source.id}:${target.id}:${index}`} className="min-w-0">
          <button type="button" aria-expanded={open} aria-controls={panelId} onClick={() => setSelected(open ? null : index)}
            className="flex w-full min-w-0 items-start gap-2 p-3 text-left hover:bg-surface-elevated focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-500">
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-1 flex flex-wrap items-center gap-2 text-xs text-ink-secondary">
                ...
                <span className={`rounded-full border px-2 py-0.5 ${negative ? "border-red-500/40 text-red-700 dark:text-red-300" : "border-outline"}`}>{status}</span>
              </span>
            </span>
          </button>
          {open && <div id={panelId} className="space-y-3 border-t border-outline bg-surface-elevated p-3 text-sm">
            {!receipt ? <p>Receipt unavailable for this hop. Collect source evidence before assessing the relationship.</p> : <>
              <dl className="grid gap-3 sm:grid-cols-2">
                ...
                ].map(([label, value]) => <div key={label}><dt className="text-xs text-ink-secondary">{label}</dt><dd className="mt-0.5 break-words">{value}</dd></div>)}
              </dl>
              <div><p className="text-xs text-ink-secondary">Source snapshots</p>
              ...
              {reasons.length ? <p className="break-words text-xs text-ink-secondary">{reasons.map(humanize).join(" · ")}</p> : null}
              <div><p className="text-xs text-ink-secondary">Runtime event references</p>
    ...
    {!visible.length && <p className="p-3 text-sm text-ink-secondary">{rows.length ? "No hops match this filter. The path is unchanged." : "This path has no relationships to inspect."}</p>}
    {filtered.length > PAGE_SIZE && <nav aria-label="Hop evidence pages" className="flex flex-wrap items-center justify-between gap-2 border-t border-outline p-3 text-sm">
