...
for (const theme of ["light", "dark"] as const) {
  test(`Context distinguishes package evidence and relationship branches in ${theme}`, async ({ page }, testInfo) => {
    ...
    await page.route("**/v1/**", route => {
      ...
      if (path.endsWith("/context-graph")) return route.fulfill({ json: { nodes, edges, lateral_paths: [], interaction_risks: [], stats: { lateral_paths_truncated: true, total_nodes: 5, total_edges: 4, agent_count: 1, shared_server_count: 0, shared_credential_count: 0, lateral_path_count: 0, max_lateral_depth: 0, highest_path_risk: 0, interaction_risk_count: 0 } } });
...
for (const scenario of [
  ...
  test(`Context neighborhood exploration stays evidence scoped ${scenario.theme} ${scenario.width}`, async ({ page }, testInfo) => {
    ...
    await inspector.getByLabel("Find loaded entity").fill("tool:deep");
    await inspector.getByRole("button", { name: /peer_only_tool.*tool:deep/ }).click();
    await expect(page.locator('.react-flow__node[data-id="tool:deep"]')).toBeAttached();
    await expect(page.locator(".react-flow__node")).toHaveCount(2);
    await page.getByRole("button", { name: "Back", exact: true }).click();
    await expect(page.locator('.react-flow__node[data-id="agent:a"]')).toBeAttached();
    await page.getByText("Advanced view", { exact: true }).click();
    await expect(page.getByLabel("Neighborhood depth", { exact: true })).toHaveValue("1");
    await page.getByLabel("Relationship direction", { exact: true }).selectOption("in");
    await expect(page.locator(".react-flow__node")).toHaveCount(1);
    await page.getByLabel("Relationship direction", { exact: true }).selectOption("both");
    await inspector.getByRole("button", { name: "Repository MCP", exact: true }).click();
    await inspector.getByRole("button", { name: "Show 1 tool · 1 hidden", exact: true }).click();
    await expect(page.locator('.react-flow__node[data-id="tool:a"]')).toBeAttached();
