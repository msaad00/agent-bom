...
/** Shared renderer names for every graph projection; matches lineageNodeTypes. */
const FLOW_RENDERER_OVERRIDES: Partial<Record<LineageNodeType, string>> = {
  "org": "providerNode",
...
export const FLOW_NODE_TYPES = Object.fromEntries(
  (Object.keys(GRAPH_LAYER_ENTITY_TYPES) as LineageNodeType[]).map(type => [type, FLOW_RENDERER_OVERRIDES[type] ?? `${type}Node`]),
