...
describe("Identity evidence source states", () => {
  it("starts with managed identities and never renders credential prefixes", async () => {
    ...
    expect(screen.getByRole("button", { name: "Managed identities" })).toHaveAttribute("aria-pressed", "true");
    ...
    expect(screen.getByRole("button", { name: "Access grants and policies" })).toHaveAttribute("aria-pressed", "true");
  });
  ...
  it("links graph identities by canonical node and returned snapshot", async () => {
    ...
    render(<IdentityPage />);
    fireEvent.click(await screen.findByRole("button", {name:"Discovered identity risk"}));
